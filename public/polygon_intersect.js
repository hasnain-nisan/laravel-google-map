...
function edgify(fig1, fig2) {
    ...
    for (var i = 0; i < primEdges.length; i++) {
        ...
        for (var j = 0; j < primEdges.length; j++) {
            if (i != j) {
                var interPoints = findEdgeIntersection(
                    primEdges[i],
                    primEdges[j]
                );
...
function polygonArea(p) {
    ...
    for (var i = 0; i < len; i++) {
        s += Math.abs(
            p[i % len].x * p[(i + 1) % len].y -
                p[i % len].y * p[(i + 1) % len].x
        );
    }
    return s / 2;
}
    ...
    while (!pointsOK) {
        ...
        for (var i = 0; i < interPoints.length - 1; i++) {
            if (interPoints[i].t !== interPoints[i + 1].t) {
                ...
                pointsOK = true;
                point = {
                    x: (interPoints[i].x + interPoints[i + 1].x) / 2,
                    y: y,
                };
            }
        }
